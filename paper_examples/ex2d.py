...
list(set.union(*[set(cyc) for cyc in mc_sol['cycles'] ]))
...
xmin = min([val[0] for val in pos.itervalues()]) - 0.5
xmax = max([val[0] for val in pos.itervalues()]) + 0.5
ymin = min([val[1] for val in pos.itervalues()]) - 0.5
ymax = max([val[1] for val in pos.itervalues()]) + 0.5

currentAxis = plt.gca()
currentAxis.add_patch(Rectangle( (-0.15, -0.15), 0.3, 0.3, color='red', alpha=0.3) )
